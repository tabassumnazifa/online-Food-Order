    ...
    public class PaymentService : IPaymentService
    {
            ...
            string transactionId)
        {
            ...
            var request = new Dictionary<string, string>
            {
                ...
                // FIX 1: Force InvariantCulture so 550 becomes "550.00" and NOT "550,00"
                { "total_amount", order.TotalAmount.ToString("0.00", CultureInfo.InvariantCulture) },
                { "currency", "BDT" },
                ...
                { "cus_city", "Dhaka" },
                { "cus_postcode", "1000" }, // FIX 2: Added mandatory postcode
                { "cus_country", "Bangladesh" },

                // Shipping information
                { "shipping_method", "Courier" },
                { "num_of_item", "1" }, // FIX 3: Added mandatory num of items
                { "ship_name", customerName },
                { "ship_add1", deliveryAddress },
                { "ship_city", "Dhaka" },
                { "ship_postcode", "1000" }, // FIX 4: Added mandatory ship postcode
            ...
            string reason)
        {
            if (payment == null)
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = "Payment information is required."
                };
            }

            if (string.IsNullOrWhiteSpace(payment.BankTransactionId))
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = "SSLCommerz bank transaction ID is missing."
                };
            }
            ...
            if (payment.Amount <= 0)
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = "Refund amount must be greater than zero."
                };
            }
            ...
            var refundTransactionId = $"REF-{payment.OrderId}-{DateTimeOffset.UtcNow.ToUnixTimeSeconds()}";

            var queryParameters = new Dictionary<string, string>
            {
                { "bank_tran_id", payment.BankTransactionId },
                { "refund_trans_id", refundTransactionId },
                // FIX: Force InvariantCulture for amount
                { "refund_amount", payment.Amount.ToString("0.00", CultureInfo.InvariantCulture) },
                { "refund_remarks", reason },
                { "store_id", _settings.StoreId },
                { "store_passwd", _settings.StorePassword },
                { "refe_id", payment.OrderId.ToString() },
                { "v", "1" },
                { "format", "json" }
            };
            ...
            var requestUrl = $"{refundUrl}?{queryString}";
            ...
            if (!response.IsSuccessStatusCode)
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = $"SSLCommerz refund request failed: {json}"
                };
            }
            ...
            if (root.TryGetProperty("status", out var status))
            {
                statusValue = status.GetString();
            }

            if (root.TryGetProperty("refund_ref_id", out var refundRefId))
            {
                refundReferenceId = refundRefId.GetString();
            }

            if (root.TryGetProperty("bank_tran_id", out var bankTranId))
            {
                bankTransactionId = bankTranId.GetString();
            }

            if (root.TryGetProperty("errorReason", out var error))
            {
                errorReason = error.GetString();
            }

            var success = string.Equals(statusValue, "success", StringComparison.OrdinalIgnoreCase)
                || string.Equals(statusValue, "processing", StringComparison.OrdinalIgnoreCase);
            ...
            string refundReferenceId)
        {
            if (string.IsNullOrWhiteSpace(refundReferenceId))
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = "Refund reference ID is required."
                };
            }
            ...
            if (!response.IsSuccessStatusCode)
            {
                return new RefundResult
                {
                    Success = false,
                    Status = "failed",
                    ErrorReason = $"SSLCommerz refund status request failed: {json}"
                };
            }
            ...
            var success = string.Equals(statusValue, "refunded", StringComparison.OrdinalIgnoreCase)
                || string.Equals(statusValue, "processing", StringComparison.OrdinalIgnoreCase);
